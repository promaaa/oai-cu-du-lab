...
For the canonical Quectel topology, `serber-firecell` hosts one shared CU.
An independent donor DU serves only the modem, while the selected access DU
serves the test phone through its local B210. Both DUs must attach to the same
CU; a monolithic donor gNB does not prove this target topology.

## Radios

- Two interchangeable B210 radios are used by the lab. The TUI discovers the
  serial connected to each selected host and injects it into the generated
  runtime config, so either unit can serve as the access or firecell radio.
- A host must expose exactly one B210 through UHD. If multiple B210 radios are
  attached, select one explicitly with the per-host serial override.
