...
\title{A Reproducible Arm-Based OpenAirInterface DU Testbed with Heterogeneous F1 Backhaul toward Multi-UAV Networks}
...
\begin{abstract}
Rapidly deployable fifth-generation (5G) coverage becomes costly when every
unmanned aerial vehicle (UAV) carries a complete x86 base station.  We instead
use F1 to keep the 5G Core and central unit (CU) on the ground and replicate
only an Arm-based OpenAirInterface (OAI) distributed unit (DU).  Raspberry Pi
...
commercial handset inside a Faraday cage.  Each setup was repeated 20 times;
mean downlink reaches
\SI{100}{\mega\bit\per\second} on tuned Ethernet,
\SI{78}{\mega\bit\per\second} over cellular backhaul, and
\SI{68}{\mega\bit\per\second} on Jetson.  Scheduler traces identify a
Block Error Rate (BLER) adaptation mismatch as the dominant observed limiter;
retuning moves the dominant Modulation and Coding Scheme (MCS) from 5 to
24--27.  A 106-PRB Raspberry Pi resource sample uses about 1.8 CPU cores and
1.4 GB RAM at \SI{64.8}{\celsius} without late-packet or overflow markers.  The
measured core electronics weigh \SI{0.657}{\kilogram}, yielding a
\SI{0.757}{\kilogram} minimum integration estimate.  The prototype is thus a
measured building block for future one-CU/many-UAV networks; flight and
...
\begin{enumerate}
  ...
  retuning; and
  \item a reproducible commercial off-the-shelf (COTS) evaluation across x86
  and Arm hosts, monolithic and split execution, and three F1 bearers, supported
  by repeated end-to-end trials, embedded-host resource observations, and
  payload-mass sizing for a lightweight UAV DU behind a shared CU.
\end{enumerate}

We intentionally do not claim that one F1 bearer is intrinsically faster than
another.  Each final setup was exercised through 20 accepted end-to-end
trials, and we report its arithmetic mean; the software, host, and RF state
evolved between setups, and the public artifact does not retain per-run
dispersion.  We also validate one DU at a time:
...
This work & OAI 5G DU; Raspberry Pi 5 or Jetson Orin Nano; B210 and 5G modem & \SI{0.657}{\kilogram} core; \SI{0.757}{\kilogram} minimum integration & 100 mean Ethernet; 78 x86 and 68 Jetson mean cellular, each $n=20$ & Itemized incremental-DU mass; flight pending. \\
\bottomrule
\end{tabularx}
\end{table*}
...
Every supported scenario was exercised in repeated deployment and end-to-end
service trials.  The handset was kept on the intended access cell, and a trial
was accepted only after PWS, registration, PDU session, Internet, clean-stop,
and rollback gates.  Interface captures established whether F1-C and F1-U used
Ethernet, GRE, or WireGuard; the cellular case additionally required encrypted
outer traffic on the modem interface.  DU logs supplied MCS, exponentially
...
Downlink (DL) and uplink (UL) rates were measured in the handset browser on
\url{https://fast.com}.  Each plotted value is the arithmetic mean of 20
accepted repetitions for that setup.  The public artifact retains the means
and sample counts, but not the per-run series, so Fig.~\ref{fig:throughput}
does not fabricate error bars.  Conditions evolved between setups; the plot
therefore establishes repeatability per setup rather than a bearer ranking.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=0.031cm,y=0.40cm,font=\scriptsize]
  \foreach \x in {0,50,100,150,200} {
    \draw[black!15] (\x,-0.45) -- (\x,8.45);
    \node[anchor=north,text=black!70] at (\x,-0.55) {\x};
  }
  \draw[->,black!65] (0,-0.45) -- (205,-0.45);
  \foreach \name/\value/\row/\shade in {
    {x86 mono}/190/8/blue,
    {x86 Ethernet}/100/7/blue,
    {x86 Wi-Fi/GRE}/52/6/blue,
    {x86 5G/WG}/78/5/blue,
    {Pi Ethernet}/21/4/green,
    {Pi Wi-Fi/GRE}/13/3/green,
    {Pi 5G/WG}/48/2/green,
    {Jetson Ethernet}/7.3/1/orange,
    {Jetson 5G/WG}/68/0/orange} {
      \node[anchor=east] at (-3,\row) {\name};
      \fill[\shade!62!black] (0,\row-0.27) rectangle (\value,\row+0.27);
      \node[anchor=west] at (\value+3,\row) {\value};
  }
  \node[anchor=north,font=\scriptsize] at (100,-1.22)
    {Mean DL throughput (Mb/s), $n=20$ per setup};
\end{tikzpicture}
\caption{Mean handset downlink over 20 accepted repetitions per setup.  The
artifact does not retain dispersion, so no error bars are shown.}
...
The final \SI{68}{\mega\bit\per\second} Jetson 5G/WireGuard mean used the full
MCS range and corrected runtime state.  The ledger keeps the earlier
\SIrange{40}{44}{\mega\bit\per\second} chronology separate rather than folding
it into the final 20-trial mean.
...
Table~\ref{tab:mcs} summarizes the observed recovery.

\begin{table}[t]
\caption{Wired-split BLER retuning and final mean throughput}
\label{tab:mcs}
\centering
\small
...
Handset DL & 12--22 Mb/s & 100 Mb/s mean ($n=20$) \\
...
the MCS table.  The final setup averaged \SI{100}{\mega\bit\per\second} across
20 accepted throughput repetitions.  The wider target is not a universal optimum: it deliberately
...
mechanism.  The wider target explains the observed MCS recovery; the throughput
rate is still reported as the outcome of the complete validated configuration,
not as a universally optimal BLER setting.

\section{Embedded Resources and Airborne Payload}
...
\SI{67.0}{\celsius}, no throttling, and no UHD underruns.  At 106 PRBs, another
sample used about 1.8 cores and 1.4 GB RAM at \SI{64.8}{\celsius}, with no
...
Complete payload power was not measured.  Component specifications and an
88\% conversion assumption estimate 19--30 W for Pi+B210+Quectel and 28--46 W
for Jetson+B210+Quectel.  They are integration bounds, not endurance results.
...
Repeated end-to-end trials validated handset-visible single-segment PWS and
data across Ethernet, Wi-Fi/GRE, and 5G/WireGuard F1 bearers.  Jetson delivered
mean \SI{68}{\mega\bit\per\second} over 5G/WireGuard ($n=20$), while the ledger retains the
intermediate \SIrange{40}{44}{\mega\bit\per\second} state.  BLER retuning
coincided with MCS 24--27 and a \SI{100}{\mega\bit\per\second} Ethernet mean
($n=20$); traces identify the BLER controller as the dominant observed limiter.
The x86 cellular-backhaul mean was \SI{78}{\mega\bit\per\second}, versus
\SI{30}{\mega\bit\per\second} in the
...
compute and thermal headroom evidence, while complete payload power remains an
estimate.  One CU can therefore amortize shared functions across lightweight
DUs as a building block for multi-UAV networks.  A uniform cross-setup
comparison retaining per-run dispersion, measured payload power, concurrent
