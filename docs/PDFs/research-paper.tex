...
\begin{abstract}
Multi-UAV fifth-generation (5G) coverage requires a lightweight, repeatable
cell building block that fleet size multiplies.  We
...
B210 while F1 traverses Ethernet, Wi-Fi/Generic Routing Encapsulation (GRE), or
5G/WireGuard.  Repeated end-to-end trials gate F1, registration, data,
rollback, and a single-segment Public Warning System (PWS) alert displayed by a
commercial handset inside a Faraday cage.  Twenty trials per fixed final
configuration in the same enclosed lab retain per-run observations; mean
downlink is
\SI{99.8}{\mega\bit\per\second} on tuned Ethernet,
\SI{76.7}{\mega\bit\per\second} over x86 cellular backhaul, and
\SI{68.4}{\mega\bit\per\second} on Jetson cellular backhaul.  Scheduler traces identify a
...
1.4 GB RAM at $64.8\,^{\circ}\mathrm{C}$ without late-packet or overflow markers.  The
measured core electronics weigh \SI{0.657}{\kilogram}.  The testbed validates
...
~\cite{kaltenberger2020}; moving only the
...
\begin{enumerate}
  ...
  displays the alert, evaluated with safe input values, the pinned OAI revision,
  and the released patch;
...
BLER changes distinguish setups.  All 200 DL observations are retained;
...
This work & OAI 5G DU; Raspberry Pi 5 or Jetson Orin Nano; B210 and 5G modem & \SI{0.657}{\kilogram} measured core & 99.8 mean Ethernet; 76.7 x86 and 68.4 Jetson mean cellular, each $n=20$ & Itemized incremental-DU mass; flight pending. \\
\bottomrule
\end{tabularx}
\end{table*}
...
Every supported scenario was exercised in repeated end-to-end trials.  The
Faraday cage, intended handset, n78 profile (30-kHz spacing, 106 PRBs), OAI pin,
and gate sequence were fixed; host and F1 bearer were the named variables.  A
trial required the intended access cell, PWS, registration, PDU session,
Internet, clean stop, and rollback.  Captures established whether F1-C/F1-U
used Ethernet, GRE, or WireGuard; the cellular case also required encrypted
modem-interface traffic.  DU logs supplied MCS, exponentially
filtered BLER, Hybrid Automatic Repeat Request (HARQ) rounds, signal-to-noise
ratio (SNR), Channel Quality Indicator (CQI) caps, and overflow counts.
Downlink (DL) and uplink (UL) throughput were measured with
\texttt{iperf3} between the UE and a server on the local data network.
F1 round-trip time (RTT) was sampled with ICMP between the CU and DU
endpoints during active traffic.  In total, 200 DL observations were collected (20 per final
configuration).  Two-sided 95\% confidence intervals (CIs) use the sample
standard deviation and Student's $t_{0.975,19}=2.093$.  The fixed lab bounds
environmental variation, but host, bearer, runtime, and BLER differences
preclude a one-factor ranking.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=0.029cm,y=0.34cm,font=\tiny]
  \foreach \x in {0,50,100,150,200} {
    \draw[black!15] (\x,-0.45) -- (\x,9.45);
    \node[anchor=north,text=black!70] at (\x,-0.55) {\x};
  }
  \draw[->,black!65] (0,-0.45) -- (205,-0.45);
  \foreach \name/\value/\half/\row/\shade in {
    {x86 mono}/190.7/0.67/9/blue,
    {x86 Ethernet}/99.8/1.18/8/blue,
    {x86 Wi-Fi/GRE}/52.0/0.42/7/blue,
    {x86 5G/WG}/76.7/2.11/6/blue,
    {Pi Ethernet}/21.9/1.57/5/green,
    {Pi Wi-Fi/GRE}/12.7/1.37/4/green,
    {Pi 5G/WG}/47.8/0.33/3/green,
    {Jetson Ethernet}/10.2/0.56/2/orange,
    {Jetson Wi-Fi/GRE}/8.1/0.24/1/orange,
    {Jetson 5G/WG}/68.4/0.70/0/orange} {
      \node[anchor=east] at (-3,\row) {\name};
      \fill[\shade!62!black] (0,\row-0.27) rectangle (\value,\row+0.27);
      \draw[black,line width=0.45pt]
        ({\value-\half},\row) -- ({\value+\half},\row);
      \draw[black,line width=0.45pt]
        ({\value-\half},\row-0.12) -- ({\value-\half},\row+0.12)
        ({\value+\half},\row-0.12) -- ({\value+\half},\row+0.12);
      \node[anchor=west] at (\value+3,\row)
        {\value\,$\pm$\,\half};
  }
  \node[anchor=north,font=\scriptsize] at (100,-1.22)
    {Mean DL throughput (Mb/s), $n=20$ per setup};
\end{tikzpicture}
\caption{Mean handset DL ($n=20$); labels and whiskers show 95\% CI
half-widths across all 200 observations.}
\label{fig:throughput}
\end{figure}

The means establish feasibility across Arm platforms and bearers without
showing that a tunneled cellular path is intrinsically faster than Ethernet.
The \SI{68.4}{\mega\bit\per\second} Jetson 5G/WireGuard mean used the full MCS
range and validated runtime state.

Table~\ref{tab:f1rtt} reports the F1 transport RTT observed during active
traffic.  Ethernet provides the tightest range; Wi-Fi/GRE adds contention and
encapsulation overhead; 5G/WireGuard traverses the donor cell and an encrypted
tunnel, roughly doubling the median latency.  These values bound the F1-C
signalling delay and F1-U forwarding budget that a scheduler encounters on each
bearer.

\begin{table}[t]
\caption{F1 round-trip time per bearer during active traffic}
\label{tab:f1rtt}
\centering
\small
\begin{tabularx}{\columnwidth}{@{}X r r@{}}
\toprule
F1 bearer & RTT range (ms) & Representative (ms) \\
\midrule
Ethernet          & 11--30  & {\raise.17ex\hbox{$\scriptstyle\sim$}}20 \\
Wi-Fi/GRE         & 15--40  & {\raise.17ex\hbox{$\scriptstyle\sim$}}25 \\
5G/WireGuard      & 25--60  & {\raise.17ex\hbox{$\scriptstyle\sim$}}40 \\
\bottomrule
\end{tabularx}
\end{table}
...
The initial Ethernet split reached at most about 23 Mb/s with MCS at its configured
...
Ethernet capacity, as the dominant observed limiter.  The same controller
behavior can arise on any bearer when its access radio shows a comparable BLER
distribution.

\subsection{BLER retuning and bounded interpretation}
...
attenuation, numerology, Bandwidth Part (BWP), F1 path, UPF, and transport
settings were unchanged.  Table~\ref{tab:mcs} records the single changed
factor and the observed response.

\begin{table}[t]
\caption{BLER-threshold intervention and final mean throughput}
\label{tab:mcs}
\centering
\small
...
Handset DL & up to 23 Mb/s & 99.8 Mb/s mean ($n=20$) \\
External-DN round-trip time & up to seconds & 11--30 ms \\
\bottomrule
\end{tabularx}
\end{table}

In a five-minute window after restart, MCS 24 and 27 were most frequent; the
scheduler was BLER-limited, not MCS-table-capped.  The retuned setup averaged
\SI{99.8}{\mega\bit\per\second} ($n=20$).  The wider target trades HARQ
retransmissions for spectral efficiency in this RF condition; the thresholds
are exposed as scenario parameters.

Filtered BLER above target followed by MCS reduction identifies the dominant
radio-layer mechanism, and the threshold-only intervention links controller
direction to MCS recovery under the recorded RF condition.  This is a
configuration-specific result, not a universal BLER target recommendation.

\section{Embedded Resources and Payload Mass}

Host-level resource measurements provide embedded execution evidence.  A 51-PRB
Raspberry Pi 5 sample recorded load 2.04, 1.5 GiB used,
$67.0\,^{\circ}\mathrm{C}$, no throttling, and no UHD underruns.  At 106 PRBs,
another sample used about 1.8 cores and 1.4 GB RAM at
$64.8\,^{\circ}\mathrm{C}$, with no
...
Estimated integration with allowance & 757.4 \\
\midrule
...
Repeated end-to-end trials in one fixed enclosed lab environment validated
handset-visible single-segment PWS and data across Ethernet, Wi-Fi/GRE, and
5G/WireGuard F1 bearers.  Jetson delivered mean
\SI{68.4}{\mega\bit\per\second} over 5G/WireGuard ($n=20$).  Changing only the
BLER thresholds moved the dominant MCS from 5 to 24--27; the retuned Ethernet
configuration averaged \SI{99.8}{\mega\bit\per\second} ($n=20$).
...
projects \SI{0.564}{\kilogram}.  Raspberry Pi resource observations provide
compute and thermal headroom evidence.  The Arm DU/radio/backhaul payload,
...
station---what fleet size multiplies.  A uniform cross-setup
matched-condition factorial comparison, measured payload power, concurrent
