...
**Status**: template for the caged Quectel monolithic-donor target. Do not mark
...
| Donor gNB | monolithic firecell donor gNB on `serber-firecell`, PCI `1`, TAC `2` |
| Donor F1 | none; donor gNB serves only the Quectel modem |
| Quectel | RM500Q-GL outside cage attached only to firecell donor gNB |
...
The local-F1 donor-DU experiment is not valid for this report.

## PASS Checklist

- [ ] firecell monolithic donor gNB is in service;
...
- [ ] minipc access DU has F1 setup to the CU over `wg-quectel-f1`;
...
| Donor gNB process/config | `<path and PID>` |
