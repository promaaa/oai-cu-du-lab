...
- **Observed User Throughput (Ethernet F1 Baseline)**: **6.1 Mbps** (initial run without MCS floor tuning).
- **Observed User Throughput (Ethernet F1 Tuned)**: **55 Mbps** (measured on Nothing Phone 5G data connection with forced MCS 10-28 and expanded BLER target window).
...
4. **MAC Scheduler MCS & BLER Tuning**: Applied expanded BLER target window (`dl_bler_target_upper=0.35`, `dl_bler_target_lower=0.25`) and forced MCS range (`10` to `28`), enabling the live radio on both Ethernet and Wi-Fi backhauls to unlock higher-order modulation (64QAM/256QAM) and reach **55 Mbps** on Ethernet and **52 Mbps** on Wi-Fi GRE.
